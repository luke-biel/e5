...
export class PacmanBackend implements Backend {
  ...
  async install(
    ...
    if (version) {
      console.log(
        yellow(
          `  Warning: Pacman does not support version pinning. Installing latest version of ${name}.`,
        ),
      );
      console.log(
        yellow(
          `  For specific versions, use a script backend with the Arch Linux Archive.`,
        ),
      );
    }
